...
//
// The free-article counter is deliberately NOT decremented: it's a LIFETIME count
// (migration 0003), so deleting never hands a slot back.

async function removeFrames(userId: string, articleId: string): Promise<void> {
  // Frames are keyed {owner}/{articleId}/... with a dense/ subfolder (worker pipeline.py
  // + storage.ts). list() isn't recursive, so sweep both levels.
  const base = `${userId}/${articleId}`
...
export async function deleteArticle(userId: string, article: ArticleRow): Promise<void> {
  const { error } = await supabase.from('articles').delete().eq('id', article.id)
  if (error) throw error

  await removeFrames(userId, article.id).catch(() => {})
