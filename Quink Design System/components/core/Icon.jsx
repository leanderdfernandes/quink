...
export function Icon({ name, size = 17, strokeWidth = 1.75, rotate, style, ...rest }) {
  const glyph = PATHS[name]
  if (!glyph) return null
  return (
    <svg width={size} height={size} viewBox="0 0 24 24" {...stroke} strokeWidth={strokeWidth} aria-hidden
      style={{ flex: 'none', transform: rotate ? `rotate(${rotate}deg)` : undefined, transition: rotate != null ? 'transform var(--dur-3) var(--ease)' : undefined, ...style }} {...rest}>
      {glyph}
    </svg>
  )
}
